...
def sequence_to_board(sequence:str) -> np.ndarray:
    ...
    for move_index, ch in enumerate(seq):
        if ch < "1" or ch > "7":
            raise ValueError(
                f"Invalid column '{ch}' at position {move_index}."
            )
        col = int(ch) - 1
        row = heights[col]
        ...
        if row >= ROWS:
            raise ValueError(
                f"Column {col + 1} is full at move index {move_index}."
            )
...
def current_player_from_sequence(sequence:str) -> int:
    """
    Return whose turn it is after the sequence:
    - 1 if player 1 to move
    - -1 if player 2 to move
    """
...
def board_from_current_player_perspective(sequence: str) -> np.ndarray:
    """
    Re-orient board so the side to move is always represented as +1.
    Opponent becomes -1.
    """
...
def board_to_pretty_string(board: np.ndarray) -> str:
    """
    Print board top-down for readability.
    X = +1
    O = -1
    . = empty
    """
...
def add_board_columns(df: pd.DataFrame, perspective: str = "raw") -> pd.DataFrame:
    """
    Adds a 'board' column containing a 6x7 numpy array.

    perspective:
    - 'raw': player 1 = 1, player 2 = -1
    - 'current': side to move = 1, opponent = -1
    """
    df = df.copy()
...
def get_legal_moves(board: np.ndarray) -> np.ndarray:
    """
    Returns a length-7 vector:
    1 if column is legal, 0 if full
    """
    legal = np.zeros(COLS, dtype=np.int8)
    for c in range(COLS):
        legal[c] = 1 if np.count_nonzero(board[:, c]) < ROWS else 0
    return legal


if __name__ == "__main__":
    df = load_dataset("UCI-Midgame-d30.train.csv")
