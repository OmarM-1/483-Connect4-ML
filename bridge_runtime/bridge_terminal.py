...
def process_operator_command(
    ...
    *,
    ...
    if cmd in {"retry", "force", "nudge"}:
        if not match_active:
            print("[bridge] No active match — retry has no effect")
            return result
        result.trigger_force_move = True
        print("[bridge] Force-move requested: clearing stuck state and re-evaluating board")
        return result

    print(
        f"[bridge] Unknown command: {cmd}. "
        "Try: pause | resume | start | status | wait <sec> | delay [x] | emote [code] | board | clear | info | retry | quit"
    )
    return result
