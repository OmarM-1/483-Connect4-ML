...
def handle_post_game_flow(
    *,
    ...
    if mode == "standby":
        ...
        # In standby+auto-rematch mode: keep retrying rematch click, never fall through.
        # Papergames shows in_game_ui=True under the rematch overlay so we cannot
        # use in_game_ui to distinguish "game active" from "rematch prompt showing".
        if auto_rematch and in_live_room and not in_lobby:
            if now - last_post_game_action_attempt_at >= action_retry_gap_sec:
                ...
                if acted:
                    print("[bridge] Standby: clicking rematch for next game")
                    reset_runtime_for_next_match_fn(post_game_waiting_empty_value=True)
                    result.post_game_waiting_empty = True
                    result.handled = True
                    return result
            if now - last_lifecycle_log >= 5.0:
                print("[bridge] Standby: post-game detected; waiting to click rematch...")
                result.last_lifecycle_log = now
            result.handled = True
            return result

        # Operator controls queue/rematch actions manually.
